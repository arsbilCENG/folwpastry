...
const SalesPurchasesPage: React.FC = () => {
  const { user } = useAuth();
  ...
  const [useProductSelect, setUseProductSelect] = useState(true);
  const [submitting, setSubmitting] = useState(false);

  const { data: purchasesData, isLoading } = usePurchases({ page, pageSize: 20 });
  
  // Purchased tipi ürünleri çek (Backend filtering applied)
  const { data: productsRes } = useQuery({
    queryKey: ['products', 'purchased'],
    queryFn: () => productApi.getProducts({ trackingType: 'Purchased' }),
  });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Error handling is managed in useMutation
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'items',
      key: 'items',
      render: (items: any[]) => `${items?.length ?? 0} ürün`,
    },
    {
      title: 'Toplam',
          ...
          <Button
            ...
            onClick={() => {
              Modal.confirm({
                ...
                content: 'Bu işlem geri alınamaz. Stok güncellemeleri de geri alınır.',
    ...
    <div style={{ padding: '24px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <Title level={3} style={{ margin: 0 }}>
          <ShoppingCartOutlined /> Satın Alımlar
        </Title>
        <Button
          type="primary"
          icon={<PlusOutlined />}
      ...
      <Modal
        title={<span><ShoppingCartOutlined /> Yeni Satın Alım</span>}
        open={isModalOpen}
        onCancel={() => { 
          if (!submitting) {
            setIsModalOpen(false); 
      ...
      >
        ...
        <Divider orientation={"left" as any}>Kalemler</Divider>

        <div style={{ background: '#fafafa', padding: 16, borderRadius: 8, marginBottom: 16, border: '1px solid #f0f0f0' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
            <Text strong>Giriş Tipi:</Text>
            <Switch
              checked={useProductSelect}
              onChange={(val) => {
                setUseProductSelect(val);
                itemForm.resetFields();
              }}
              checkedChildren="Sistemden Seç"
              unCheckedChildren="Serbest Giriş"
            />
            {useProductSelect && (
              <Tag color="success" icon={<CheckCircleOutlined />}>Stoğa Eklenecek</Tag>
            )}
          </div>
          ...
          <Form form={itemForm} layout="vertical">
            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', gap: '8px', alignItems: 'start' }}>
              {useProductSelect ? (
                <Form.Item name="productId" label="Ürün" rules={[{ required: true, message: 'Seçiniz' }]}>
                  <Select
                    showSearch
                ...
                <Form.Item name="itemName" label="Gider Adı" rules={[{ required: true, message: 'Giriniz' }]}>
                  <Input placeholder="Kırtasiye, odun, vb..." />
                </Form.Item>
              )}
            ...
            <Table
              ...
              columns={[
                ...
                },
                {
                  ...
                  key: 'stock',
                  align: 'center',
                  render: (pid) => pid ? <Tag color="success">✓</Tag> : <Tag color="default">-</Tag>,
                },
                {
                  title: '',
                  key: 'action',
            ...
            <Table
              ...
              columns={[
                ...
                },
                {
                  ...
                  key: 'stock',
                  align: 'center',
                  render: (v) => v ? <Tag color="success">Stokta</Tag> : <Tag color="default">Gider</Tag>,
                },
              ]}
            />
